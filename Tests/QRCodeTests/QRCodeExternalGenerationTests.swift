...
final class QRCodeExternalGenerationTests: XCTestCase {
	...
	func testNumericEncoding() throws {
		...
		let docN1 = QRCode.Document(utf8String: v1, generator: eGen)
		XCTAssertEqual(23, docN1.boolMatrix.dimension)
		// Setting as data, therefore not optimized
		let docN2 = QRCode.Document(data: try XCTUnwrap(v1.data(using: .utf8)), generator: eGen)
	...
	func testOptimizationWorkingNumerics() throws {
		...
		let origDoc = QRCode.Document(data: data, generator: eGen)
		...
		let optDoc = QRCode.Document(utf8String: text, generator: eGen)
